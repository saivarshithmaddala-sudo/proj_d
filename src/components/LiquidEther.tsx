...
export default function LiquidEther({
  ...
  const webglRef = useRef<WebGLManagerHandle | null>(null);
  ...
  useEffect(() => {
    ...
    interface FBOMap { [key: string]: THREE.WebGLRenderTarget }
    ...
    class ShaderPass {
      ...
      constructor(props: Record<string, unknown>) {
        this.props = props || {};
        this.uniforms = (this.props.material as Record<string, unknown>)?.uniforms as Record<string, THREE.IUniform> | undefined;
      }
      init() {
    ...
    class Advection extends ShaderPass {
      ...
      constructor(simProps: { cellScale: THREE.Vector2; fboSize: THREE.Vector2; dt: number; src: THREE.WebGLRenderTarget; dst: THREE.WebGLRenderTarget }) {
        ...
        this.uniforms = (this.props.material as Record<string, unknown>).uniforms as Record<string, THREE.IUniform>;
        this.init();
      }
      init() {
        super.init();
    ...
    class ExternalForce extends ShaderPass {
      ...
      initForce(simProps: { cellScale: THREE.Vector2; cursor_size: number; dst: THREE.WebGLRenderTarget }) {
        super.init();
    ...
    class Viscous extends ShaderPass {
      constructor(simProps: { cellScale: THREE.Vector2; boundarySpace: THREE.Vector2; viscous: number; src: THREE.WebGLRenderTarget; dst: THREE.WebGLRenderTarget; dst_: THREE.WebGLRenderTarget; dt: number }) {
      ...
      update({ viscous: v, iterations, dt }: { viscous: number; iterations: number; dt: number }) {
    ...
    class Divergence extends ShaderPass {
      constructor(simProps: { cellScale: THREE.Vector2; boundarySpace: THREE.Vector2; src: THREE.WebGLRenderTarget; dst: THREE.WebGLRenderTarget; dt: number }) {
      ...
      update({ vel }: { vel: THREE.WebGLRenderTarget }) { this.uniforms!.velocity.value = vel.texture; super._render(); }
    }

    class Poisson extends ShaderPass {
      constructor(simProps: { cellScale: THREE.Vector2; boundarySpace: THREE.Vector2; src: THREE.WebGLRenderTarget; dst: THREE.WebGLRenderTarget; dst_: THREE.WebGLRenderTarget }) {
    ...
    class Pressure extends ShaderPass {
      constructor(simProps: { cellScale: THREE.Vector2; boundarySpace: THREE.Vector2; src_p: THREE.WebGLRenderTarget; src_v: THREE.WebGLRenderTarget; dst: THREE.WebGLRenderTarget; dt: number }) {
    ...
    class Simulation {
      ...
      createShaderPass() {
        const f = this.fbos;
        this.advection = new Advection({ cellScale: this.cellScale, fboSize: this.fboSize, dt: this.options.dt, src: f.vel_0, dst: f.vel_1 });
        this.externalForce = new ExternalForce({ cellScale: this.cellScale, cursor_size: this.options.cursor_size, dst: f.vel_1 });
        this.viscousPass = new Viscous({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, viscous: this.options.viscous, src: f.vel_1, dst: f.vel_viscous1, dst_: f.vel_viscous0, dt: this.options.dt });
        this.divergence = new Divergence({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src: f.vel_viscous0, dst: f.div, dt: this.options.dt });
        ...
        this.pressurePass = new Pressure({ cellScale: this.cellScale, boundarySpace: this.boundarySpace, src_p: f.pressure_0, src_v: f.vel_viscous0, dst: f.vel_0, dt: this.options.dt });
      }
      calcSize() {
    ...
    webglRef.current = webgl as unknown as WebGLManagerHandle;
    webgl.start();
    ...
    const io = new IntersectionObserver(entries => {
      ...
      if (isVis && !document.hidden) webglRef.current.start();
